...
/** GET /v1/sessions/{id}/clips、/v1/clips/{cid} 返回的切片 */
 ...
 */
export async function downloadClip(clip: Clip, format: 'source' | 'mp4'): Promise<void> {
...
interface ToolsStatus {
  ffmpeg: { available: boolean; source: string; path: string | null; version: string | null; error: string | null }
}

/**
 * 精确剪、非 MP4 产物的 MP4 下载要用服务器上的 ffmpeg。`reason` 为 null 表示可用，
 * 否则是给按钮提示用的原因。
 */
export function useFfmpeg(): { ready: boolean; reason: string | null } {
  ...
  return {
    ready: false,
    reason: `服务器上的 ffmpeg 不可用${detail}。精确剪和转成 MP4 要用它：安装 ffmpeg，或在配置里把 ffmpeg_path 指向它，然后刷新页面。快速剪和源格式下载不受影响`,
  }
}
